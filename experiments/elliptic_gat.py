#!/usr/bin/env python3
"""Elliptic Bitcoin fraud detection with GAT."""
...
def main():
    # Load config
    config_path = Path(__file__).parent.parent / "configs" / "elliptic_gat.yaml"
    config = OmegaConf.load(config_path)

    # Setup
    set_seed(config.seed)
    device = get_device()
    run_id = generate_run_id("elliptic", "gat")
    ...
    print(f"\nTraining GAT for {config.training.epochs} epochs...")
    results = trainer.train(log_interval=20)
    trainer.save(run_id, "GAT", "elliptic")
